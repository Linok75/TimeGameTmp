...
if(isset($_POST['valeur'])){
	...
	if($titre!=""){
		$selectManga=$dbh->prepare('SELECT idanime FROM Anime Where titre LIKE "'.bdSql($titre).'"');
		...
		try {
			...
			if ($selectManga->fetchColumn() > 0) {
				...
				while ($ligne = $selectManga->fetch()) {
					echo 'Une fiche sur cet anime existe déjà ici : <a href="./index.php?module=Fiche&action=displayFiche&idanime='.html($ligne[0]).'">'.$titre.'</a>';
				}
			}else {
				echo $valide;
			}
